...
interface HippoSwapFactory {
    function createPair(address tokenA, address tokenB) external returns (address pair);
}

interface HippoSwapRouter {
...
contract FLOKIC is Ownable, IBEP20 {
    using SafeMath for uint256;

    uint8 constant private _decimals = 18;

    uint256 private _totalSupply = 1000000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = _totalSupply * 20 / 1000; //Sell Only
    ...
    address private hippoAddress = 0x8F2a57bbC2B3870Ed321541F091f9A3EbE008Ad4;

    string constant private _name = "KungPoW";
    string constant private _symbol = "KPOW";
    ...
    event AutoLiquify(uint256 amountETC, uint256 amountBOG);
    ...
    constructor() {
        router = HippoSwapRouter(hippoAddress);
        pair = HippoSwapFactory(router.factory()).createPair(router.WETH(), address(this));
        ...
        totalFee = liquidityFee.add(marketingFee).add(utilityFee).add(nativeFee);
        totalFeeIfSelling = totalFee;

        _alances[msg.sender] = _totalSupply;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "TX Limit Exceeded");
        if (!isPair[msg.sender] && !inSwapAndLiquify && swapAndLiquifyEnabled && _balances[address(this)] >= swapThreshold) {marketingAndLiquidity();}
    ...
    function marketingAndLiquidity() internal lockTheSwap {
        ...
        uint256 amountETC = address(this).balance;

        uint256 totalETCFee = totalFee.sub(nativeFee).sub(liquidityFee.div(2));

        uint256 amountETCLiquidity = amountETC.mul(liquidityFee).div(totalETCFee).div(2);
        uint256 amountETCMarketing = amountETC.mul(marketingFee).div(totalETCFee);
        uint256 amountETCUtility = amountETC.mul(utilityFee).div(totalETCFee);
        
        (bool tmpSuccess1,) = payable(marketingWallet).call{value : amountETCMarketing, gas : 30000}("");
        tmpSuccess1 = false;

        (tmpSuccess1,) = payable(utilityWallet).call{value : amountETCUtility, gas : 30000}("");
        tmpSuccess1 = false;

        if (amountToLiquify > 0) {
            router.addLiquidityETH{value : amountETCLiquidity}(
                address(this),
                amountToLiquify,
                0,
                0,
            ...
            emit AutoLiquify(amountETCLiquidity, amountToLiquify);
        }
    }
    ...
    function tradingStatus(bool newStatus) public onlyOwner {
        require(canUseBlacklist, "Utility can no longer pause trading");
    ...
    function setIsPair(address pool, bool status) external onlyOwner {
        isPair[pool] = status;
        if(status == true){
            _allowances[address(this)][address(pool)] = type(uint256).max;
        }
    }
    ...
    function setFees(uint256 newLiqFee, uint256 newMarketingFee, uint256 newUtilityFee, uint256 newNativeFee, uint256 extraSellFee) external onlyOwner {
    ...
    function enable_blacklist(bool _status) public onlyOwner {
        require(canUseBlacklist, "Utility can no longer add blacklists");
    ...
    function setFeeReceivers(address newMktWallet, address newUtilityWallet, address newLpWallet) public onlyOwner{
